{{ html()->element('footer')->class('sticky bottom-0 z-30 h-16 max-h-16 w-full bg-secondary-950 border-t border-secondary-800/80')->children([
    html()->div()->class('container flex h-16 items-center justify-between gap-x-3 overflow-x-auto sm:justify-center sm:gap-x-12')->children([
        html()->a()->route('home')->text('Home')->prependChild(
            html()->icon('heroicon-o-square-2-stack')->class('size-5')
        ),

        html()->a()->href(route('home'))->text('Home'),

        html()->a()->href(route('home'))->text('Home'),

        html()->a()->href(route('home'))->text('Home'),
    ])
]) }}
