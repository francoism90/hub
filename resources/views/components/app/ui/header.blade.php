{{ html()->element('header')->class('h-14 min-h-14 w-full bg-gray-950 border-b border-gray-800/80')->children([
    html()->element('nav')->class('container mx-0 max-w-none h-full flex flex-nowrap items-stretch justify-between gap-x-3 overflow-x-auto sm:gap-x-12')->children([
        html()->div()->class('inline-flex w-2/4 items-center justify-start')->child(
            html()->a()->link('home')->children([
                html()->div()->class('flex flex-nowrap items-center text-pink-100 hover:text-pink-100 gap-x-3')->children([
                    html()->icon()->svg('heroicon-s-play-circle', 'size-8 bg-linear-to-r from-indigo-500 via-purple-500 to-pink-500 p-0.5'),
