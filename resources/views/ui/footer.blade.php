{{ html()->element('footer')->class('sticky bottom-0 z-30 h-16 max-h-16 w-full bg-secondary-950 border-t border-secondary-800/80')->children([
    html()->element('nav')->class('*:flex-col *:p-1.5 *:gap-1.5 *:line-clamp *:text-xs *:font-medium container flex h-full justify-between gap-x-3 overflow-x-auto sm:gap-x-12')->children([
